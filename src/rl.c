...
char *
pal_rl_no_match (void)
{
  return NULL;
}

void
pal_rl_default_text_fn (void)
{
  gchar *locale_default_text
      = g_locale_from_utf8 (pal_rl_default_text, -1, NULL, NULL, NULL);
...
gchar *
pal_rl_get_raw_line (const char *prompt, const int row, const int col)
{
  char *line = NULL;
  ...
  if (!g_utf8_validate (line, -1, NULL))
    {
      ...
      if (utf8_string == NULL)
        {
          pal_output_error (
              _ ("WARNING: Failed to convert your input into UTF-8.\n"));
          return line;
        }
      else
        {
...
gchar *
pal_rl_get_line (const char *prompt, const int row, const int col)
{
  gchar *line = NULL;
  do
    {
...
gchar *
pal_rl_get_line_default (const char *prompt, const int row, const int col,
...
void
pal_rl_completions_output (char **matches, int num_matches, int max_length)
{
  ...
  for (i = 0; i < num_matches - 1; i++)
    {
      move ((y + 1) + (i / matches_per_line),
            (i % matches_per_line) * max_length);
...
gboolean
pal_rl_get_y_n (const char *prompt)
{
  gchar *s = NULL;
...
PalEvent *
pal_rl_get_event (GDate **d, gboolean allow_global)
{
  gchar *s = NULL;
  PalEvent *event = NULL;
  *d = NULL;
  ...
  while (1)
    {
      ...
      pal_output_wrap (_ ("Valid date formats include: yyyymmdd, Jan 1 2000, "
                          "1 Jan 2000, 4 days away"),
                       2, 2);

      s = pal_rl_get_line (_ ("Date for event or search string: "),
                           settings->term_rows - 2, 0);
          ...
          while (1)
            {
              pal_output_fg (BRIGHT, YELLOW, "> ");
              pal_output_wrap (
                  _ ("Use \"0\" to use a different date or search string."), 2,
                  2);

              s = pal_rl_get_line (_ ("Select event number: "),
                                   settings->term_rows - 2, 0);
              if (strcmp (s, "0") == 0)
              ...
              if (event != NULL)
                {
                  ...
                  pal_output_wrap (
                      _ ("This event is in a global calendar file.  You can "
                         "change this event only by editing the global "
                         "calendar file manually (root access might be "
                         "required)."),
                      2, 2);
                }
            }
          ...
          while (1)
            {
              ...
              event = pal_search_event_num (event_num, d, search_string, date,
                                            365);
              if (event != NULL)
                {
                  if (!event->global || allow_global)
