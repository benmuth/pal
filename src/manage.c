      ...
      if (thisdaycount > 0 || isselectedday)
        {
          ...
          linecount += pal_output_date (date, TRUE,
                                        isselectedday ? selected_event : -1);
  ...
  if (selected_event >= 0 && events_on_day > 0)
    {
      ...
      pal_curwin = subwin (stdscr, 5, saved_cols / 2, settings->cal_lines + 4,
                           saved_cols / 2);

      curevent = g_list_nth_data (events,
                                  (selected_event >= 0) ? selected_event : 0);
      g_list_free (events);
      ...
      if (curevent->start_date)
        g_date_strftime (date_text, 128, settings->date_fmt,
                         curevent->start_date);
      else
...
pal_manage_isearch_refresh (void)
{
  ...
  g_date_strftime (buffer, 128, settings->date_fmt, selected_day);

  pal_output_fg (BRIGHT, GREEN, "%s Isearch starting from %s",
                 isearch_direction ? "Forward" : "Backward", buffer);
  g_print (" ");

  /* Only search if there is text to search for */
  if (*rl_line_buffer
      && !pal_search_isearch_event (&searchdate, &searchselect, rl_line_buffer,
                                    isearch_direction))
    {
...
static void
pal_manage_isearch (gboolean forward)
{
  ...
  if (*searchstring)
    {
      if (pal_search_isearch_event (&searchdate, &searchselect, rl_line_buffer,
                                    isearch_direction))
        {
...
static void
pal_manage_scan_for_event (GDate **date, int *eventnum, int dir)
{
...
pal_manage (void)
{
  ...
  /* initialize curses */
  (void)signal (SIGINT,
                pal_manage_finish); /* arrange interrupts to terminate */
  ...
  (void)halfdelay (
      1);          /* always return from getch() within a tenth of a second */
  (void)noecho (); /* echo input - in color */

  signal (SIGINT, pal_manage_finish); /* setup function to call on Ctrl+C */
  signal (SIGWINCH,
          pal_manage_resize); /* setup function to call on term resize */
  ...
  for (;;)
    {
      ...
      switch (c)
        ...
        case 'G':
          {
            gchar *str = pal_rl_get_raw_line ("Goto date: ", 0, 0);
        ...
        case 'E':
          ...
          if (selected_event != -1)
            {
              PalEvent *e
                  = pal_output_event_num (selected_day, selected_event + 1);
              if (e != NULL)
                {
                  if (e->global)
                    {
                      ...
                      pal_output_fg (BRIGHT, RED,
                                     "Can't edit global event!");
                    }
                  else
                    {
                      //			    gchar* new_text =
                      // pal_edit_desc(e->text);
                      gchar *new_text = pal_rl_get_line_default (
                          "New description: ", 0, 0, e->text);

                      pal_del_write_file (e);
                      pal_add_write_file (e->file_name, e->date_string,
                                          new_text);
              ...
              if (e != NULL)
                {
                  ...
                  if (e->global)
                    pal_output_fg (BRIGHT, RED,
                                   "Can't delete global event!");
                  else
                    {
                      if (pal_rl_get_y_n ("Are you sure you want to delete "
                                             "this event? [y/n]: "))
                        {
                          selected_event = -1;
                          pal_del_write_file (e);
        ...
        case 'H':
          ...
          pal_output_fg (BRIGHT, GREEN, "LeftArrow or h");
          g_print (" - %s\n", "Back one day");

          pal_output_fg (BRIGHT, GREEN, "RightArrow");
          g_print (" - %s\n", "Forward one day or l");

          pal_output_fg (BRIGHT, GREEN, "UpArrow or k");
          g_print (" - %s\n", "Back one week or event (if in "
                                 "event selection mode)");

          pal_output_fg (BRIGHT, GREEN, "DownArrow or j");
          g_print (" - %s\n", "Forward one week or event (if in "
                                 "event selection mode)");
