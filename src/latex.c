...
static void
pal_latex_escape_print (gchar *s)
{
  gunichar c;
...
static void
pal_latex_month (GDate *date, gboolean force_month_label)
{
  gint i;
  gchar buf[1024];
  ...
  g_print ("%s", "\\begin{tabularx}{10.0in}{|p{1.26in}|p{1.26in}|p{1.26in}|p{"
                 "1.26in}|p{1.26in}|p{1.26in}|p{1.24in}|}\n");
  g_print ("%s", "\\hline\n");
  ...
  while (g_date_get_month (date) == orig_month)
    {
      ...
      while (num_events > num_events_printed)
        {
          gchar *event_text
              = pal_event_escape ((PalEvent *)(item->data), date);
          g_print ("$\\cdot$");
...
pal_latex_out (void)
{
  ...
  g_print ("%s",
           "%% Uncomment this line if you want you have unicode output\n");
