...
static int use_colors
    = -1; /* -2 = no colors, can't turn them on later if this is set;
             -1 = don't know;
              0 = no colors;
              1 = colors */
...
color_term (void)
{
  ...
  if (getenv ("COLORTERM") != NULL || !g_ascii_strncasecmp (term, "xterm", 5)
...
void
colorize_xterm_title (gchar *title)
{
  if (use_colors == -1)
    color_term ();
...
static const char *string_colors[] = { "black", "red",     "green", "yellow",
